...
// Import travel data
const { destinations, aircraft, saveTrips, loadTrips } = require('./travelData.js');
...
app.get('/weather', (req, res) => {
    ...
    fetch(`https://api.weatherbit.io/v2.0/forecast/daily?lat=${lat}&lon=${lon}&key=${process.env.WEATHERBIT_API_KEY}`)
        ...
        .catch((err) => {
            console.error(err);
            console.log(`Failed to acquire weather data for lat: ${lat} long: ${lon}`);
        });
});
...
app.get('/geo', (req, res) => {
    ...
    fetch(`http://api.geonames.org/searchJSON?placename=${city}&country=${country}&username=${process.env.GEONAMES_USERNAME}`)
        .then((response) => response.json())
        .then((data) => {
        ...
        .catch((err) => {
            console.error(err);
            console.log(`Failed to acquire data for places starting with ${city}`);
        });
});
...
app.get('/images', (req, res) => {
    ...
    fetch(`https://pixabay.com/api/?q=${q}&key=${process.env.PIXABAY_API_KEY}`)
        .then((response) => response.json())
        .then((data) => {
...
app.get('/trips', async (req, res) => {
    if (tripsData.length > 0) {
        ...
        try {
            ...
            console.error(err);
            console.log("Failed to add loaded trips to tripsData");
        }
    }
});
